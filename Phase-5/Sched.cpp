			...
			for (size_t i = 0; i < numElements; i++)
			{
				...
				{//allow head to run
				  ...
				  write_window(processTable->taskWin, buff);
					...
					//Removed quantum from this If statement
					if(processTable->state == RUNNING  || processTable->state == DEAD || processTable->state == BLOCKED){
					  if (processTable != processTail)//more than 2 elements
						...
						if (processTable->state != DEAD && processTable->state != BLOCKED)
						{
							processTable->state = RUNNING;
							break;
						}
